...
encodeMultiAssetMaps ::
  ...
  MultiAsset crypto ->
  Encoding
encodeMultiAssetMaps (MultiAsset v) =
  encodeMap toCBOR (encodeMap toCBOR toCBOR) v
...
from (CompactValueAdaOnly (CompactCoin c)) = Value (fromIntegral c) (MultiAsset mempty)
...
insert ::
  ...
  Value crypto ->
  Value crypto
insert combine pid aid new (Value cn (MultiAsset m1)) =
        ...
        (v1, Just old, v2) ->
          if n == 0
            then
              let m3 = link2 v1 v2
               in if Map.null m3
                    then Value cn (MultiAsset (link2 l1 l2))
                    else Value cn (MultiAsset (link pid m3 l1 l2))
            else Value cn (MultiAsset (link pid (link aid n v1 v2) l1 l2))
          where
            n = combine old new
        (_, Nothing, _) ->
          Value
...
valueFromList ada =
  foldr
    ...
    (Value ada (MultiAsset Map.empty))
...
gettriples' (Value c (MultiAsset m1)) = (c, triples, bad)
  where
    triples =
