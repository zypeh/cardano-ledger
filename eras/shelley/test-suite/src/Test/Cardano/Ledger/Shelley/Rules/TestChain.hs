  ...
  where
    ...
    sumIncreaseRewards
      SourceSignalTarget
        ...
        (coinBalance u' <+> d' <+> f')
          <-> (coinBalance u <+> d <+> f)
          === (UM.fromCompact (sumRewardsView (UM.Rewards umap1)))
          <-> (UM.fromCompact (sumRewardsView (UM.Rewards umap2)))
...
depositInvariant SourceSignalTarget {source = chainSt} =
  let LedgerState {lsUTxOState = utxost, lsDPState = DPState dstate pstate} = (esLState . nesEs . chainNes) chainSt
      ...
      keyDeposits = (UM.fromCompact . UM.sumDepositView . UM.Rewards . dsUnified) dstate
