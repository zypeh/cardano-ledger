...
alonzoSpecificProps SourceSignalTarget {source = chainSt, signal = block} =
  ...
  where
    ...
    alonzoSpecificPropsLEDGER
      SourceSignalTarget
        { source = LedgerState ShelleyUTxOState {sutxosUtxo = UTxO u, sutxosDeposited = dp, sutxosFees = f} ds,
          signal = tx,
          target = LedgerState ShelleyUTxOState {sutxosUtxo = UTxO u', sutxosDeposited = dp', sutxosFees = f'} ds'
        } =
