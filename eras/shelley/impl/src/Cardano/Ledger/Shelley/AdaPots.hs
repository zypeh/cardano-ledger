...
import Cardano.Ledger.UMapCompact (View (Rewards), sumDepositView, sumRewardsView)
...
totalAdaPotsES (EpochState (AccountState treasury_ reserves_) _ ls _ _ _) =
  ...
  where
    ...
    coins = coinBalance u
    keyDeposits_ = (fromCompact . sumDepositView . Rewards . dsUnified . dpsDState . lsDPState) ls
