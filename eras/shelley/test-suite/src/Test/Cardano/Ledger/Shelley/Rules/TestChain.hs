...
incrStakeComp SourceSignalTarget {source = chainSt, signal = block} =
  ...
  where
    ...
    checkIncrStakeComp
      SourceSignalTarget
        { source = LedgerState ShelleyUTxOState {sutxosUtxo = u, sutxosStakeDistr = sd} dp,
          signal = tx,
          target = LedgerState ShelleyUTxOState {sutxosUtxo = u', sutxosStakeDistr = sd'} dp'
        } =
        counterexample
          ( mconcat
              ( [ "\nDetails:\n",
...
checkPreservation (SourceSignalTarget {source, target, signal}, count) =
  ...
  where
    ...
    proposal = votedValue (proposals . sutxosPpups . lsUTxOState $ lsOld) currPP 5
...
utxoDepositsIncreaseByFeesWithdrawals SourceSignalTarget {source, signal, target} =
  ...
  where
    ...
    circulation chainSt =
      let ShelleyUTxOState {sutxosUtxo = u, sutxosDeposited = d} = us chainSt
       in coinBalance u <+> d
...
potsSumIncreaseWdrlsPerBlock SourceSignalTarget {source, signal, target} =
  ...
  where
    potsSum chainSt =
      let ShelleyUTxOState {sutxosUtxo = u, sutxosDeposited = d, sutxosFees = f} =
...
potsSumIncreaseWdrlsPerTx SourceSignalTarget {source = chainSt, signal = block} =
  ...
  where
    ...
    sumIncreaseWdrls
      SourceSignalTarget
        { source = LedgerState ShelleyUTxOState {sutxosUtxo = u, sutxosDeposited = d, sutxosFees = f} _,
          signal = tx,
          target = LedgerState ShelleyUTxOState {sutxosUtxo = u', sutxosDeposited = d', sutxosFees = f'} _
        } =
  ...
  where
    ...
    sumIncreaseRewards
      SourceSignalTarget
        { source =
            LedgerState
              ShelleyUTxOState {sutxosUtxo = u, sutxosDeposited = d, sutxosFees = f}
  ...
  where
    ...
    createdIsConsumed SourceSignalTarget {source = ledgerSt, signal = tx, target = ledgerSt'} =
      ...
      where
        ...
        LedgerState (ShelleyUTxOState {sutxosUtxo = u}) dpstate = ledgerSt
        LedgerState (ShelleyUTxOState {sutxosUtxo = u'}) _ = ledgerSt'
        txb = tx ^. bodyTxL
        created =
          coinBalance u'
            <+> txb
...
preserveBalanceRestricted SourceSignalTarget {source = chainSt, signal = block} =
  ...
  where
    ...
    createdIsConsumed
      SourceSignalTarget
        { source = LedgerState (ShelleyUTxOState {sutxosUtxo = UTxO u}) dpstate,
          signal = tx
        } =
        inps === outs
        where
...
preserveOutputsTx SourceSignalTarget {source = chainSt, signal = block} =
  ...
  where
    ...
    outputPreserved
      SourceSignalTarget
        { target = LedgerState (ShelleyUTxOState {sutxosUtxo = UTxO utxo}) _,
          signal = tx
        } =
...
canRestrictUTxO SourceSignalTarget {source = chainSt, signal = block} =
  ...
  where
    ...
    outputPreserved
      SourceSignalTarget {target = LedgerState (ShelleyUTxOState {sutxosUtxo = UTxO uFull}) _}
      SourceSignalTarget {target = LedgerState (ShelleyUTxOState {sutxosUtxo = UTxO uRestr}) _} =
        counterexample
...
eliminateTxInputs SourceSignalTarget {source = chainSt, signal = block} =
  ...
  where
    ...
    inputsEliminated
      SourceSignalTarget
        { target = LedgerState (ShelleyUTxOState {sutxosUtxo = (UTxO u')}) _,
          signal = tx
        } =
        property $
          hasFailedScripts tx
...
newEntriesAndUniqueTxIns SourceSignalTarget {source = chainSt, signal = block} =
  ...
  where
    ...
    newEntryPresent
      SourceSignalTarget
        { source = LedgerState (ShelleyUTxOState {sutxosUtxo = UTxO u}) _,
          signal = tx,
          target = LedgerState (ShelleyUTxOState {sutxosUtxo = UTxO u'}) _
        } =
        let UTxO outs = txouts @era (tx ^. bodyTxL)
...
txFees ledgerTr =
  ...
  where
    f
      c
      SourceSignalTarget
        { source = LedgerState ShelleyUTxOState {sutxosUtxo = utxo} _,
          signal = tx
...
nonNegativeDeposits SourceSignalTarget {source = chainSt} =
  let es = (nesEs . chainNes) chainSt
      ShelleyUTxOState {sutxosDeposited = d} = (lsUTxOState . esLState) es
...
depositInvariant SourceSignalTarget {source = chainSt} =
  let LedgerState {lsUTxOState = sutxost, lsDPState = DPState dstate pstate} = (esLState . nesEs . chainNes) chainSt
...
feesNonDecreasing SourceSignalTarget {source, target} =
  ...
  where
    fees_ chainSt =
      let ShelleyUTxOState {sutxosFees = fees} =
            lsUTxOState . esLState . nesEs . chainNes $ chainSt
       in fees
...
ledgerTraceFromBlockWithRestrictedUTxO chainSt block =
  ...
  where
    ...
    ledgerSt0' = LedgerState (utxoSt {sutxosUtxo = UTxO relevantUTxO}) delegationSt
...
testIncrementalStake ::
  ...
  LedgerState era ->
  Property
testIncrementalStake _ (LedgerState (ShelleyUTxOState utxo _ _ _ incStake) (DPState dstate pstate)) =
