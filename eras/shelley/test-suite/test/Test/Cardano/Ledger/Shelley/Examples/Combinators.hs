...
feesAndDeposits ppEx newFees stakes pools cs = cs {chainNes = nes'}
  where
    ...
    dpstate' =
      DPState
        dstate {dsUnified = UM.unionKeyDeposits (Rewards (dsUnified dstate)) newDeposits}
...
feesAndKeyRefund newFees key cs = cs {chainNes = nes'}
  where
    ...
    ls = esLState es
    DPState dstate pstate = lsDPState ls
    refund = case UM.lookup key (Rewards (dsUnified dstate)) of
      Nothing -> Coin 0
    ...
    es' = es {esLState = ls'}
    nes' = nes {nesEs = es'}
    dpstate' = DPState dstate {dsUnified = UM.adjust zeroD key (Rewards (dsUnified dstate))} pstate
      ...
      ds
        { dsUnified =
            let um0 = dsUnified ds
                um1 = (UM.insert cred (UM.RDPair (UM.CompactCoin 0) (UM.CompactCoin 0)) (Rewards um0))
