...
import Cardano.Ledger.UMapCompact (View (Rewards), depositView, domain, fromCompact, sumDepositView)
...
depositInvariant SourceSignalTarget {source = mockChainSt} =
  let LedgerState {lsUTxOState = utxost, lsDPState = DPState dstate pstate} = (esLState . nesEs . mcsNes) mockChainSt
      ...
      keyDeposits = fromCompact $ sumDepositView (Rewards (dsUnified dstate))
