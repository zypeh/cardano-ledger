...
incrStakeComp SourceSignalTarget {source = chainSt, signal = block} =
  ...
  where
    ...
    checkIncrStakeComp
      SourceSignalTarget
        { source = LedgerState UTxOState {sutxosUtxo = u, sutxosStakeDistr = sd} dp,
          signal = tx,
          target = LedgerState UTxOState {sutxosUtxo = u', sutxosStakeDistr = sd'} dp'
        } =
        counterexample
          ( mconcat
              ( [ "\nDetails:\n",
...
utxoDepositsIncreaseByFeesWithdrawals SourceSignalTarget {source, signal, target} =
  ...
  where
    ...
    circulation chainSt =
      let UTxOState {sutxosUtxo = u, sutxosDeposited = d} = us chainSt
       in coinBalance u <+> d
...
potsSumIncreaseWdrlsPerBlock SourceSignalTarget {source, signal, target} =
  ...
  where
    potsSum chainSt =
      let UTxOState {sutxosUtxo = u, sutxosDeposited = d, sutxosFees = f} =
...
potsSumIncreaseWdrlsPerTx SourceSignalTarget {source = chainSt, signal = block} =
  ...
  where
    ...
    sumIncreaseWdrls
      SourceSignalTarget
        { source = LedgerState UTxOState {sutxosUtxo = u, sutxosDeposited = d, sutxosFees = f} _,
          signal = tx,
          target = LedgerState UTxOState {sutxosUtxo = u', sutxosDeposited = d', sutxosFees = f'} _
        } =
  ...
  where
    ...
    sumIncreaseRewards
      SourceSignalTarget
        { source =
            LedgerState
              UTxOState {sutxosUtxo = u, sutxosDeposited = d, sutxosFees = f}
  ...
  where
    ...
    createdIsConsumed SourceSignalTarget {source = ledgerSt, signal = tx, target = ledgerSt'} =
      ...
      where
        ...
        LedgerState (UTxOState {sutxosUtxo = u}) dpstate = ledgerSt
        LedgerState (UTxOState {sutxosUtxo = u'}) _ = ledgerSt'
        txb = tx ^. bodyTxL
        created =
          coinBalance u'
            <+> txb
...
preserveBalanceRestricted SourceSignalTarget {source = chainSt, signal = block} =
  ...
  where
    ...
    createdIsConsumed
      SourceSignalTarget
        { source = LedgerState (UTxOState {sutxosUtxo = UTxO u}) dpstate,
          signal = tx
        } =
        inps === outs
        where
...
preserveOutputsTx SourceSignalTarget {source = chainSt, signal = block} =
  ...
  where
    ...
    outputPreserved
      SourceSignalTarget
        { target = LedgerState (UTxOState {sutxosUtxo = UTxO utxo}) _,
          signal = tx
        } =
...
canRestrictUTxO SourceSignalTarget {source = chainSt, signal = block} =
  ...
  where
    ...
    outputPreserved
      SourceSignalTarget {target = LedgerState (UTxOState {sutxosUtxo = UTxO uFull}) _}
      SourceSignalTarget {target = LedgerState (UTxOState {sutxosUtxo = UTxO uRestr}) _} =
        counterexample
...
eliminateTxInputs SourceSignalTarget {source = chainSt, signal = block} =
  ...
  where
    ...
    inputsEliminated
      SourceSignalTarget
        { target = LedgerState (UTxOState {sutxosUtxo = (UTxO u')}) _,
          signal = tx
        } =
        property $
          hasFailedScripts tx
...
newEntriesAndUniqueTxIns SourceSignalTarget {source = chainSt, signal = block} =
  ...
  where
    ...
    newEntryPresent
      SourceSignalTarget
        { source = LedgerState (UTxOState {sutxosUtxo = UTxO u}) _,
          signal = tx,
          target = LedgerState (UTxOState {sutxosUtxo = UTxO u'}) _
        } =
        let UTxO outs = txouts @era (tx ^. bodyTxL)
...
txFees ledgerTr =
  ...
  where
    f
      c
      SourceSignalTarget
        { source = LedgerState UTxOState {sutxosUtxo = utxo} _,
          signal = tx
...
nonNegativeDeposits SourceSignalTarget {source = chainSt} =
  let es = (nesEs . chainNes) chainSt
      UTxOState {sutxosDeposited = d} = (lsUTxOState . esLState) es
